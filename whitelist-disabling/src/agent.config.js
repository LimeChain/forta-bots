...
module.exports = {
  getContractAndNetwork: async () => {
    ...
    switch (chainId) {
      case 1: {
        const contract = new ethers.Contract(ETHEREUM_CONTRACT_ADDRESS, ABI, getEthersProvider());
        return { contract, network: 'Ethereum' };
      }
      case 137: {
        const contract = new ethers.Contract(POLYGON_CONTRACT_ADDRESS, ABI, getEthersProvider());
        return { contract, network: 'Polygon' };
      }
      default:
